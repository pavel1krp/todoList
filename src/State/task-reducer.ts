...
export const changeTAskStatusAC = (todoListId: string, taskId: string, done: boolean) => ({
    type: "CHANGE-IS-DONE",
    todoListId,
    taskId,
    done
...
export const changeTaskTitle = (todoListId:string,taskId:string)=>(dispatch:Dispatch, getStata: ()=>AppRootStateType)=>{
    let task = getStata().task
    todolistAPI.updateTaskS(todoListId, taskId)
        .then(res=> changeTAskStatusAC(todoListId,taskId,true))
}